<#
.SYNOPSIS
Exports a Microsoft 365 and Entra group inventory.
.DESCRIPTION
Reports group type, mail state, visibility, membership type, creation date, and member count.
This script performs read-only Microsoft 365 reporting through Microsoft Graph PowerShell. It does not modify tenant data.
.PARAMETER OutputDirectory
Directory where CSV output is written.
.PARAMETER DisconnectWhenComplete
Disconnects the current Microsoft Graph session after the report finishes.
.EXAMPLE
.\Get-M365GroupInventory.ps1
.NOTES
Required delegated scopes: Group.Read.All
Some data may require an appropriate Microsoft 365 or Microsoft Entra license and administrator consent.
#>
[CmdletBinding()]
param(
    [ValidateNotNullOrEmpty()]
    [string]$OutputDirectory = (Join-Path $PSScriptRoot '..\..\Reports\CSV'),

    [switch]$DisconnectWhenComplete
)

Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

function Connect-RequiredGraphScope {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string[]]$Scopes)

    if (-not (Get-Module -ListAvailable -Name Microsoft.Graph.Authentication)) {
        throw 'Microsoft Graph PowerShell is required. Install with: Install-Module Microsoft.Graph -Scope CurrentUser'
    }

    Import-Module Microsoft.Graph.Authentication -ErrorAction Stop
    $context = Get-MgContext
    $missing = @($Scopes | Where-Object { -not $context -or $_ -notin $context.Scopes })
    if ($missing.Count -gt 0) {
        Write-Verbose "Connecting to Microsoft Graph with scopes: $($Scopes -join ', ')"
        Connect-MgGraph -Scopes $Scopes -NoWelcome | Out-Null
    }
}

function New-ReportPath {
    param([Parameter(Mandatory)][string]$Name)
    New-Item -ItemType Directory -Path $OutputDirectory -Force | Out-Null
    Join-Path $OutputDirectory ("{0}-{1}.csv" -f $Name, (Get-Date -Format 'yyyyMMdd-HHmmss'))
}

$requiredScopes = @('Group.Read.All')
Connect-RequiredGraphScope -Scopes $requiredScopes
$uri = "https://graph.microsoft.com/v1.0/groups?`$select=id,displayName,description,groupTypes,mailEnabled,securityEnabled,mail,visibility,membershipRule,createdDateTime&`$top=999"
$groups = @()
do {
    $response = Invoke-MgGraphRequest -Method GET -Uri $uri
    $groups += $response.value
    $uri = $response.'@odata.nextLink'
} while ($uri)
$report = foreach ($group in $groups) {
    $countResponse = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/groups/$($group.id)/members/`$count" -Headers @{ ConsistencyLevel='eventual' }
    [pscustomobject]@{
        DisplayName      = $group.displayName
        Description      = $group.description
        GroupType        = if ('Unified' -in $group.groupTypes) { 'Microsoft 365' } elseif ($group.mailEnabled) { 'Distribution/Mail-enabled' } else { 'Security' }
        MembershipType   = if ('DynamicMembership' -in $group.groupTypes) { 'Dynamic' } else { 'Assigned' }
        MailEnabled      = $group.mailEnabled
        SecurityEnabled  = $group.securityEnabled
        Mail             = $group.mail
        Visibility       = $group.visibility
        MemberCount      = [int]$countResponse
        CreatedDateTime  = $group.createdDateTime
    }
}
$path = New-ReportPath -Name 'M365-GroupInventory'
$report | Sort-Object GroupType,DisplayName | Export-Csv $path -NoTypeInformation -Encoding UTF8
Write-Output $report
Write-Host "Report saved to $path"
if ($DisconnectWhenComplete) { Disconnect-MgGraph | Out-Null }
