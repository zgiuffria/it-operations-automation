<#
.SYNOPSIS
Exports a Microsoft 365 user account overview.
.DESCRIPTION
Reports account status, user type, department, title, creation date, and assigned license count.
...
.EXAMPLE
.\Get-M365UserOverview.ps1
.NOTES
Required delegated scopes: User.Read.All
...
$uri = "https://graph.microsoft.com/v1.0/users?`$select=displayName,userPrincipalName,accountEnabled,userType,department,jobTitle,createdDateTime,assignedLicenses&`$top=999"
$users = @()
do {
...
$report = $users | ForEach-Object {
    [pscustomobject]@{
        ...
        AssignedLicenses  = @($_.assignedLicenses).Count
    }
}
$path = New-ReportPath -Name 'M365-UserOverview'
$report | Sort-Object UserPrincipalName | Export-Csv -Path $path -NoTypeInformation -Encoding UTF8
Write-Output $report
